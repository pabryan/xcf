...
\]
and the Weyl tensor \(\weyl\) contains the most complexity of \(\Rm\) - ie. that part not arising from the indecomposable element \(-g \owedge P\) built from the Ricci tensor \(\Ric = c(\Rm)\). Let us also note that \(\weyl \equiv 0\) if and only if \((M, g)\) is locally conformally flat.

In particular, in dimensions \(n = 2, 3\), \(\weyl \equiv 0\), so \(\Rm\) is a much simpler object. The Ricci decomposition as written is only valid when \(n > 2\), but when \(n = 2\), \(\Trfree{\Ric} \equiv 0\) and so if we make the convention \(\tfrac{1}{n-2} \Trfree{\Ric} = 0\) for \(n = 2\), the decomposition holds. In dimension \(n = 4\), the Weyl tensor decomposes further as \(\weyl = \weyl^+ + \weyl^-\) into self-dual and anti-self dual parts. For \(n > 5\) it is indecomposable under the action of \(O(n)\). Let us record explicitly,
\begin{align*}
...
For completeness, we also introduce here the Ricci-Einstein tensor, \(\RicEin\),
\[
...
\]
and the adjugate Einstein tensor,
\[
\adjg{\Ein} (X, Y) = g(\adjg{\edoEin} (X), Y).
\]
These tensors will be discussed in more detail in the next section where they are used to define our flows.
