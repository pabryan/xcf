...
#+BEGIN_proof
In the case the \(\edoEin\) is invertible at the point \(x\), this is straight forward: since \(\Ein\) is symmetric, we have \(\edoEin\) is self adjoint. The for any \(X, Y \in T_x M\) we have
\begin{align*}
g(\edoEin^{-1} (X), Y) &= g(\edoEin^{-1} (X), \edoEin \circ \edoEin^{-1} (Y)) = g(\edoEin \circ \edoEin^{-1} (X), \edoEin^{-1}(Y)) \\
&= g(X, \edoEin^{-1} (Y))
\end{align*}
so that \(\edoEin^{-1}\) is self adjoint, hence so too is \(\adjg{\edoEin} = \det \edoEin \edoEin^{-1}\) and hence
\begin{align*}
\adjg{\Ein}(X, Y) &= g(\det \edoEin \edoEin^{-1} (X), Y) = \det \edoEin g(X, \edoEin^{-1}(Y)) \\
&= g(X, \det\edoEin \edoEin^{-1}(Y)) = \adjg{\Ein}(Y, X).
\end{align*}

In fact, this is the only case we require in what follows since there we will assume \(\edoEin_x\) is invertible for all \(x \in M\), but we give the general proof here anyway to be complete.

That is now assume \(\edoEin_x\) is not invertible so that \(\det \edoEin_x = 0\) and \(\ker \edoEin_x \ne \emptyset\). Then we have the splitting
\[
T_x M = \ker \edoEin_x \oplus \ker^{\perp} \edoEin_x
\]
where \(\ker^{\perp} \edoEin_x\) is the \(g_x\)-orthogonal complement to \(\ker \edoEin_x\). Moreover, \(\ker^{\perp} \edoEin_x = \text{image} (\edoEin_x)\), since firstly for any \(X\) and any \(Y \in \ker \edoEin_x\) we have
\[
g(\edoEin_x(X), Y) = g(X, \edoEin_x(Y)) = 0
\]
so that \(\edoEin_x(X) \perp Y\) and hence \(\edoEin_x(X) \in \ker^{\perp} \edoEin_x\). Secondly, \(\ker^{\perp} \edoEin_x \cap \ker \edoEin_x = \{0\}\) so that \(\edoEin_x|_{\ker^{\perp} \edoEin_x} : \ker^{\perp} \edoEin_x \to \ker^{\perp} \edoEin_x\) is injective and hence surjective. Thus any \(X\) may be written uniquely as
\[
X = \edoEin(Z) + X_0
\]
where \(\edoEin(X_0) = 0\) and \(Z \in \ker^{\perp} \edoEin_x\).

Next for any \(Y \in \ker \edoEin_x\), let \(\edoEin_x' = \edoEin_x|_{Y^{\perp}}\) where \(Y^{\perp}\) is the \(g\) orthogonal complement of the one-dimensional subspace spanned by \(Y\). Then we claim that
\[
...
Now to see the claim, first observe that \(\adjg{\edoEin_x}|_{\ker^{\perp} \edoEin_x} \equiv 0\) since for any \(X \in \ker^{\perp} \edoEin_x\),
\[
...
\]
If \(\dim \ker \edoEin_x = 1\), then we are done since then \(Y\) spans \(\ker \edoEin_x\). If \(\dim \ker \edoEin_x > 1\), then taking any \(X \in \ker \edoEin_x\) \(g\) linearly independent of \(Y\), we may write \(\sigma = X \wedge \sigma'\) by taking for example \(\sigma'\) to span \(\Lambda^{n-2} V\) where \(V\) is any complement to the span of \(X\) and \(Y\). Hence,
\[
...
We have already shown the first case is self adjoint and the third case, being the zero map is self adjoint. The second case is also self adjoint because orthogonal projections are always self adjoint since we can uniquely write \(X = X' + X^{\perp}\) with \(X' \in \ker \edoEin\) and \(X^{\perp} \in \ker^{\perp} \edoEin\) so that
\begin{align*}
