...
#+BEGIN_proof
In the case the \(\edoEin\) is invertible this is straight forward: since \(\Ein\) is symmetric, we have \(\edoEin\) is self adjoint. The for any \(X, Y\) we have
\[
g(\edoEin^{-1} (X), Y) = g(\edoEin^{-1} (X), \edoEin \circ \edoEin^{-1} (Y)) = g(\edoEin \circ \edoEin^{-1} (X), \edoEin^{-1}(Y)) = g(X, \edoEin^{-1} (Y))
\]
so that \(\edoEin^{-1}\) is self adjoint, hence so too is \(\adjg{\edoEin} = \det \edoEin \edoEin^{-1}\) and hence
\[
\adjg{\Ein}(X, Y) = g(\det \edoEin \edoEin^{-1} (X), Y) = \det \edoEin g(X, \edoEin^{-1}(Y)) = g(X, \det\edoEin \edoEin^{-1}(Y)) = \adjg{\Ein}(Y, X).
\]

In fact, this is the only case we require in what follows, but we give the general proof to be complete.
#+END_proof
